...
async function saveDemoResumeTemplate() {
  try {
    // Connect to MongoDB
    const mongoUri = process.env.MONGODB_URI || 'mongodb+srv://balwinder_cvgenix_1998:mAxGheQuqWAyvmzc@cvgenixdb.vrkl6u1.mongodb.net/?retryWrites=true&w=majority&appName=cvgenixdb';
    ...
    const demoResumeTemplate = new Template({
      ...
      thumbnail: "/assets/images/templates/professional-thumbnail.jpg",
      preview: "/assets/images/templates/professional-preview.jpg",
      renderEngine: "canvas",
      isActive: true,
      isPremium: false,
      isPopular: true,
    ...
    console.log('📋 Template Name:', demoResumeTemplate.name);
    console.log('📋 Objects Count:', demoResumeTemplate.canvasData.objects.length);

    // Verify the template was saved
    const savedTemplate = await Template.findById(demoResumeTemplate._id);
    console.log('✅ Template verification successful');
    console.log('📊 Template details:', {
      id: savedTemplate._id,
      name: savedTemplate.name,
      renderEngine: savedTemplate.renderEngine,
      objectsCount: savedTemplate.canvasData.objects.length
    });
